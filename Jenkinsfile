    ...
    stage("Deploy to kubernetes") {
      steps {
        withCredentials([file(credentialsId: "jenkins-kubeconfig", variable: "KUBECONFIG")]) {
          sh "helm secrets upgrade --install $HELM_RELEASE_NAME $HELM_CHART_NAME --namespace $KUBERNETES_NAMESPACE --set image.repository=$DOCKER_REGISTRY_REPOSITORY --set image.tag=$IMAGE_TAG"
        }
      }
    }
  }
